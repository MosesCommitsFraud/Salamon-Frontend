/*
 * PreviewMessage component – live Reasoning mode (fixed syntax errors)
 */
...
import { YugiohCardGallery } from '@/components/chatcard';
import GlowingLogo from '@/components/logoSvg';
import ResultList from '@/components/resultlist';

function extractReasoning(raw: string): {
    cleanedText: string;
    ...
    while (cursor < raw.length) {
        const idxOpen = raw.indexOf(openTag, cursor);
        if (idxOpen === -1) {
            cleanedParts.push(raw.slice(cursor));
            break;
        }
        cleanedParts.push(raw.slice(cursor, idxOpen));
        const startContent = idxOpen + openTag.length;
        const idxClose = raw.indexOf(closeTag, startContent);
        if (idxClose === -1) {
            streamingReasoning = raw.slice(startContent).trim();
            break;
        }
        reasoningBlocks.push(raw.slice(startContent, idxClose).trim());
        cursor = idxClose + closeTag.length;
    }

    const cleanedText = cleanedParts.join('').trim();
    return { cleanedText, reasoningBlocks, streamingReasoning };
}
                ...
                >
                    {message.role === 'assistant' && (
                        <div className="size-8 flex items-center rounded-full justify-center ring-1 shrink-0 ring-border bg-background">
                            <div className="translate-y-px mt-1">
                                <GlowingLogo size={28} />
                            </div>
                        </div>
                    )}
                    ...
                    <div className="flex flex-col gap-4 w-full">
                        ...
                        {message.parts?.map((part, index) => {
                            ...
                            if (part.type === 'text') {
                                const { cleanedText, reasoningBlocks, streamingReasoning } =
                                    extractReasoning(part.text);
                                if (mode === 'view') {
                                    return (
                            ...
                            if (part.type === 'tool-invocation') {
                                ...
                                if (state === 'result') {
                                    ...
                                    const cardIds = Array.isArray(result) ? result.map((c: any) => c.id) : [];
                                    return (
                                        <div key={toolCallId}>
                                            {toolName === 'getInformation' ? (
                                                <YugiohCardGallery cardIds={cardIds} />
                                            ) : toolName === 'getRules' ? (
